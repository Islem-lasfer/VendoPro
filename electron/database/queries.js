...
const createProduct = (product) => {
  ...
  executeModify(`
    INSERT INTO products (barcode, name, category, price, detailPrice, wholesalePrice, expirationDate, quantity, quantityType, purchasePrice, image, serialNumber, incomplete, addedFrom, createdAt)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `, [
...
const updateProduct = (id, product) => {
  ...
  executeModify(`
    ...
    SET barcode = ?, name = ?, category = ?, price = ?, detailPrice = ?, wholesalePrice = ?, expirationDate = ?, quantity = ?, quantityType = ?,
        purchasePrice = ?, image = ?, serialNumber = ?, incomplete = ?, updatedAt = CURRENT_TIMESTAMP
    WHERE id = ?
  `, [
    product.barcode || null,
    product.name,
  ...
  try {
    ...
    db.prepare(`
      INSERT INTO invoices (
        invoiceNumber, subtotal, discount, tax, total, paymentMethod, customerName, clientAddress, clientEmail, clientPhone, clientId, paymentStatus, companyName, companyAddress, companyContact, companyTaxId, garantieDuration, garantieEndDate, notes, type, source, date, debt, paid
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run([
...
const createSupplierInvoice = (invoice) => {
  ...
  try {
    ...
    db.prepare(`
      INSERT INTO supplier_invoices (invoiceNumber, supplierName, date, total, paid, debt, status, notes)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run([
      invoice.invoiceNumber,
  ...
  try {
    ...
    if (Array.isArray(snapshot.locations)) {
      ...
      snapshot.locations.forEach(loc => {
        db.prepare(`INSERT INTO locations (id, name, type, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?)`).run([loc.id || null, loc.name || null, loc.type || null, loc.createdAt || null, loc.updatedAt || null]);
      });
    }
    ...
    if (Array.isArray(snapshot.products)) {
      ...
      snapshot.products.forEach(p => {
        db.prepare(`INSERT INTO products (id, barcode, name, category, price, detailPrice, wholesalePrice, expirationDate, quantity, quantityType, purchasePrice, image, serialNumber, incomplete, addedFrom, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run([
    ...
    if (Array.isArray(snapshot.supplierInvoices)) {
      ...
      snapshot.supplierInvoices.forEach(inv => {
        db.prepare(`INSERT INTO supplier_invoices (id, invoiceNumber, supplierName, date, total, paid, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run([inv.id || null, inv.invoiceNumber || null, inv.supplierName || null, inv.date || null, inv.total || 0, inv.paid || 0, inv.createdAt || null, inv.updatedAt || null]);
        if (Array.isArray(inv.items)) {
          inv.items.forEach(item => {
            db.prepare(`INSERT INTO supplier_invoice_items (id, supplierInvoiceId, productId, productName, quantity, quantityType, purchasePrice, total) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run([item.id || null, inv.id, item.productId || null, item.productName || null, item.quantity || 0, item.quantityType || 'unit', item.purchasePrice || 0, item.total || 0]);
          });
        }
      });
    }
    ...
    if (Array.isArray(snapshot.employees)) {
      ...
      snapshot.employees.forEach(emp => {
        db.prepare(`INSERT INTO employees (id, name, role, salary, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?)`).run([emp.id || null, emp.name || null, emp.role || null, emp.salary || null, emp.createdAt || null, emp.updatedAt || null]);
